...
Verified live example: [Policy research report for the Interim Measures for Generative AI Services](docs/analysis_result_example.zh-CN.md). It was generated by run `run-20260806T110626-e72098de7073` and contains five impact paths and twelve audited company-path matches. The web UI serves this same file at `/example-report`.

PolicyChain is an Agentic RAG system for policy research. Users submit a policy URL or full policy text; the system validates the source, analyzes the policy, compares similar policies, derives implementation and industry-impact paths, and produces evidence-bound A-share company business matches.
...
- Company Matcher runs DeepSeek and Web Search independently for each impact path to discover named A-share seeds, then uses CNFinancial exact name/code/profile queries to verify identity and enrich business evidence. Web-only fallback requires two independent sources; explicit identity conflicts remain a veto.
...
Company matching defaults to Web-first discovery. The discovery pool keeps up to five candidates per impact path for evaluation, while the deterministic final report displays at most three:

```powershell
$env:POLICYCHAIN_COMPANY_DISCOVERY_MODE='web_first'
$env:POLICYCHAIN_MCP_MAX_COMPANY_CANDIDATES='5'
$env:POLICYCHAIN_MAX_COMPANY_MATCHES_PER_IMPACT='3'
```

Use `POLICYCHAIN_COMPANY_DISCOVERY_MODE=legacy_cnfinancial` only as an explicit rollback switch.
...
- Web-first Company Matcher with per-impact DeepSeek/Web discovery, exact CNFinancial identity verification, evidence fusion, deterministic auditing, and an absolute three-company-per-impact report cap.
...
- Structured Company Matcher seed, identity, evidence, audit, ranking, fallback, and coverage observability.
...
- Dedicated exchange, CNInfo, and company-official-source connectors beyond the current Web Search and CNFinancial channels.
